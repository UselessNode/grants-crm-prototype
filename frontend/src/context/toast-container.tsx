...
// Константы стилей (вынесены за компонент)
...
// Позиционирование (можно расширить)
...
export function ToastContainer({ position = 'top-right' }: ToastContainerProps) {
  ...
  return (
    <div
      className={`fixed ${positionClasses[position]} z-[9999] flex flex-col gap-3 max-w-sm w-full pointer-events-none`}
      role="status"
      aria-live="polite"
      aria-atomic="true"
    >
      {toasts.map((toast) => {
        ...
        // Анимация: добавляем класс exiting для исчезновения
        ...
        return (
          <div
            key={toast.id}
            className={`pointer-events-auto ${styles.bg} border-l-4 ${styles.border} rounded-lg shadow-lg p-4 transition-all duration-300 ease-in-out ${animationClass}`}
            role="alert"
          >

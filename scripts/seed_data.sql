...
(4, 'Подготовка', 'Старт регистрации команд', 'Открытие приёма заявок на сайте и в соцсетях', '2026-04-01', '2026-04-15', '50+ зарегистрированных команд', 'Скриншоты формы, реестр'),
(4, 'Проведение', 'Финальный матч', 'Торжественное закрытие и награждение', '2026-06-15', '2026-06-15', 'Определение победителей, фотоотчёт', 'Протокол, фото, видео');
...
UPDATE applications SET expert_1 = 1, expert_2 = 2 WHERE id = 1; -- Эко-тропа
UPDATE applications SET expert_1 = 3, expert_2 = 4 WHERE id = 4; -- Спортивный фестиваль

-- Вердикты экспертов
INSERT INTO expert_verdicts (application_id, expert_id, verdict, comment, created_at) VALUES
(1, 1, 'approved', 'Проект соответствует критериям. Бюджет обоснован.', '2026-03-05 10:00:00'),
(1, 2, 'approved', 'Рекомендую к поддержке. Команда мотивирована.', '2026-03-05 11:30:00'),
(4, 3, 'approved', 'Перспективный проект. Есть потенциал масштабирования.', '2026-03-10 14:00:00'),
(4, 4, 'rejected', 'Бюджет завышен. Требуется доработка плана мероприятий.', '2026-03-10 15:30:00'),
...
-- 6. Документы и шаблоны согласий
-- ========================================

-- Примечание: Бинарные данные файлов (file_data) добавляются через API
-- Здесь создаются только записи для демонстрации структуры

-- Образец согласия для совершеннолетних (14+)
INSERT INTO documents (title, description, category_id, file_data, file_name, file_type, file_size, is_template, template_type, created_by, created_at) VALUES
(
  'Образец согласия на обработку персональных данных (14+)',
  'Шаблон согласия для участников проекта старше 14 лет',
  5, -- Образцы согласий
  E'\\x255044462d312e34', -- Заглушка: %PDF-1.4
  'consent_adult_sample.pdf',
  'application/pdf',
  245000,
  true,
  'consent_adult',
...
-- Примеры других документов (с заглушками вместо реальных файлов)
INSERT INTO documents (title, description, category_id, file_data, file_name, file_type, file_size, is_template, created_by, created_at) VALUES
(
